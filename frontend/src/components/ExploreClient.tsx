    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-none border-2 border-foreground/20 bg-card py-3 pl-10 pr-20 text-xs font-mono text-foreground placeholder-muted focus:border-foreground focus:outline-none transition-all"
        />
        {searchQuery && (
          <button
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-y-2 border-foreground/20 bg-card/60 p-3 sm:p-4">
            ...
            <button
              ...
              className={`rounded-none px-3 py-1.5 text-xs font-mono shrink-0 transition-colors uppercase tracking-wider font-bold ${
                activeCategory === c.key
                  ...
                  : 'bg-background border border-foreground/20 text-muted hover:text-foreground hover:border-foreground'
              }`}
            >
              {c.label}
            </button>
        ...
        <div className="flex items-center gap-2 self-end sm:self-auto font-mono text-xs uppercase tracking-wider font-bold">
          ...
          <button
            ...
            className="rounded-none border border-foreground/20 bg-background px-2.5 py-1 text-muted hover:text-foreground hover:border-foreground"
          >
      ...
      {/* Results Header */}
      <div className="flex items-center justify-between text-xs font-mono text-muted uppercase tracking-wider font-bold border-b border-foreground/20 pb-2">
        <div>
      ...
      <div className="space-y-4">
        {filteredItems.map((item) => (
          <div
            ...
            className="rounded-none border-2 border-foreground/20 bg-card p-4 sm:p-5 transition-all hover:bg-card-hover"
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-2 border-b border-foreground/20/20 pb-2">
                ...
                {item.tags && item.tags.length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {item.tags.slice(0, 4).map((tag) => (
                      <span
                        key={tag}
                        className="font-mono text-[10px] uppercase tracking-wider font-bold text-foreground border border-foreground/20 px-1.5 py-0.5 rounded-none bg-background"
                      >
                        {tag}
                      </span>
                    ))}
              ...
              <div className="flex flex-col items-end shrink-0 gap-3 border-l border-foreground/20/20 pl-4">
        ...
        {filteredItems.length === 0 && (
          <div className="py-12 text-center font-mono text-xs uppercase tracking-wider font-bold text-muted border-2 border-dashed border-foreground/20 rounded-none">
